...
from fairmotion.utils import utils as utils2
...
def fix_model_keys(d):
  old_keys = list(d.keys())
  for old_key in old_keys: 
    new_key =  str(old_key)[7:]
...
def prepare_model(path, num_predictions, args, device):
    ...
    ph = torch.load(path)
    ph = fix_model_keys(ph)
    model.load_state_dict(ph)
    model.eval()
    return model
...
def save_motion_files(seqs_T, args):
    ...
    utils2.create_dir_if_absent(os.path.join(args.save_output_path, "ref"))
    utils2.create_dir_if_absent(os.path.join(args.save_output_path, "pred"))
