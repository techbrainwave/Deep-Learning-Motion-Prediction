...
def train(args):
    ...
    if args.multi_gpu:
        ...
        logging.info("multi GPU ON: {} GPUs".format(len(device_ids)))
        model = torch.nn.DataParallel(model, device_ids=device_ids)
        model = model.to(device)
    print(model)
    logging.info(msg="model build finish")
    ...
    for epoch in range(args.epochs):
        ...
        teacher_forcing_ratio = np.clip(
            (1 - epoch / args.epochs), a_min=0.1, a_max=1,
        )
        logging.info(
        ...
        for iterations, (src_seqs, tgt_seqs) in enumerate(dataset["train"]):
            ...
            epoch_loss += loss.item()
            if iterations%10==0:
                logging.info(f"iter:{iterations}, loss:{loss.item():.4f}")
        epoch_loss = epoch_loss / num_training_sequences
    ...
    logging.info(f"Validation MAE:\n{mae_plot}")
    with open(os.path.join(args.save_model_path, "mae.txt"), 'w') as f:
        for ind, mae in enumerate(mae_plot):
            f.write('epoch ' + str(ind) + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ninp",
        type=int,
        help="model size of transformer",
        default=128,
    )
    parser.add_argument(
        "--num-heads",
        type=int,
        help="number of heads in multihead transformer",
        default=8,
    )
    parser.add_argument(
        "--num-layers",
